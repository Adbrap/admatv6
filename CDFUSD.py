...
def Finder_IETE(time1, time_name1, start1):
    ...
    with my_lock:
        ...
        api_url_livePrice = f'http://api.polygon.io/v1/last_quote/currencies/{tiker_live}?apiKey={api_key}'
        ...
        for k in range(0, len(df_livePrice.index)):
            if df_livePrice.index[k] == 'ask':
                la_place_de_ask = k
            if df_livePrice.index[k] == 'bid':
                la_place_de_bid = k

        livePrice = (df_livePrice['last'][la_place_de_bid] + df_livePrice['last'][la_place_de_ask])/2
    ...
    print('--- Mode recherche CDFUSD', time1, time_name1, ' ---', flush=True)
